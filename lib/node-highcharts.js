...
function createHighchartsWindow(callback) {
	var window 	= jsdom.jsdom().defaultView,
		...
		jquery = window.document.createElement('script');
	...
	jquery.src = 'file:///' + __dirname + '/highcharts/jquery-1.4.2.min.js';
	...
	window.document.head.appendChild(jquery);
}
...
function render(options, callback) {
	createHighchartsWindow(function(window) {
      ...
      if (document.errors && document.errors.length > 0) {
        return callback(_.map(document.errors, function (item) {
          if (_.isError(item.data.error)) {
